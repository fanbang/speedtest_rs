...
async fn main() {
	...
	let bg = warp::path("bg.jpg").map(|| {
        let file = Asset::get("bg.jpg").unwrap();
        reply::html(file)
    }).with(reply::with::header(header::CONTENT_TYPE, "image/jpg"));
    ...
    let route = index_html.or(results_html).or(worker).or(ping)
        .or(download).or(upload).or(report).or(results).or(favicon).or(bg);
